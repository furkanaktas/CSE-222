...
public class HuffmanTree
    ...
    /**
     * 
     * @param target  şifrelenecek string
     * @return  şifrelenmiş kod(binary)
     */
    public String encode(String target){

        int size = target.length(),count;
        count = size;
        for (int i = 0; i < size; ++i)
        {
            int tempSize = encoded.size();
            String temp = target.substring(i,i+1);
            helpEncode( temp,"", huffTree);
            if(encoded.size()-tempSize != 1 ){
                System.out.println("'"+temp+"'"+" karakteri ağaçta yok! Encode edilmedi.");
                --count;
            }
    ...
    /** Outputs the resulting code.
     @param target  character which is search for.
     @param code The code up to this node
     @param tree The current node in the tree
     */
    private void helpEncode( String target,String code, BinaryTree < HuffData > tree) {
        HuffData theData = tree.getData();
        if (theData.symbol != null) {

            if(theData.symbol.equals(target.charAt(0))) {
                encoded.add(code);
            }
        }
        else {
            helpEncode(target,code + "0", tree.getLeftSubtree());
            helpEncode( target,code + "1", tree.getRightSubtree());
        }
    }
